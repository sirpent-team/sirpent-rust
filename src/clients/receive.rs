...
pub fn clients_receive(mut clients: HashMap<I, C>) -> ClientsReceive<I, C, BoxFuture<(Msg, C), ()>> {
    let clients = clients.into_iter();
    for (id, command_tx) in clients {
        ...
        let command_future = command_tx.send(client_cmd).map_err(|_| ());





        // If queueing did not error, store it back into self.
