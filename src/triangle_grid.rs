...
impl Direction for TriangleDir {
    fn variants() -> &'static [TriangleDir] {
        static VARIANTS: &'static [TriangleDir] = &[TriangleDir::East, TriangleDir::South, TriangleDir::West];
        VARIANTS
    }
}
...
impl Vector for TriangleVector {
    ...
    fn distance(&self, other : &TriangleVector) -> usize {
        ...
        let d3 = (((self.u + self.v + (self.r as isize))) - ((other.u + other.v + (other.r as isize)))).abs();
        (du + dv + d3) as usize
    }
    ...
    fn neighbour(&self, direction : &TriangleDir) -> TriangleVector {
        match self.r {
            true => match *direction {
                TriangleDir::East  => TriangleVector {u : self.u + 1, v : self.v    , r : false},
                TriangleDir::South => TriangleVector {u : self.u    , v : self.v + 1, r : false},
                TriangleDir::West  => TriangleVector {u : self.u    , v : self.v    , r : false},
            },
            false => match *direction {
                TriangleDir::East  => TriangleVector {u : self.u    , v : self.v    , r : true},
                TriangleDir::South => TriangleVector {u : self.u    , v : self.v - 1, r : true},
                TriangleDir::West  => TriangleVector {u : self.u - 1, v : self.v    , r : true},
            },
        }
    }
...
mod tests {
    ...
    fn triangle_inequality() {
        quickcheck(triangle_inequality_prop as fn(TriangleVector, TriangleVector, TriangleVector) -> bool);
    }
