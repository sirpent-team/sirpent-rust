...
mod tests {
    ...
    impl Arbitrary for Snake {
        fn arbitrary<G: Gen>(g: &mut G) -> Snake {
            ...
            let head: Vector = Vector::Hexagon(Arbitrary::arbitrary(g));
            let segments = (0..size)
                .scan(head, |state, _| {
                    let dir = Direction::Hexagon(Arbitrary::arbitrary(g));
    ...
    fn step_preserves_connectedness_prop(snake: Snake, dir: HexagonDirection) -> bool {
        let mut snake = snake.clone();
        snake.step_in_direction(Direction::Hexagon(dir));
        return snake_is_connected_prop(snake);
    }

    #[test]
    fn step_preserves_connectedness() {
        quickcheck(step_preserves_connectedness_prop as fn(Snake, HexagonDirection) -> bool);
    }
